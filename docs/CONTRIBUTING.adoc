...
This scripts uses `git`, `gh`, `jq` and `mvn` to generate the release.
The script also requires Bash version to be greater than 4.
...
The project doesn't publish Java binary.
...
Once it is merged, a deployment will be trigger through a pull request on link:https://github.com/jenkins-infra/kubernetes-management[] repository.
Only when this second pull request is merge that the new release is deployed on the infra system and available at link:https://plugin-health.jenkins.io[].
