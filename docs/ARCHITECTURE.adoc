...
This is done in link:../war/src/main/java/io/jenkins/pluginhealth/scoring/schedule/DefaultUpdateCenterScheduler.java[`DefaultUpdateCenterScheduler`].
...
The `ProbeEngine` is scheduled by the link:../war/src/main/java/io/jenkins/pluginhealth/scoring/schedule/DefaultProbeEngineScheduler.java[`DefaultProbeEngineScheduler`] class.
...
The execution of the `ScoringEngine` is schedule by the `DefaultProbeEngineScheduler`, just after the `ProbeEngine` is finished.
